...
bool deleteFlightDataFile(uint8_t chipSelect) {
  if (!initSDCard(chipSelect)) return false;
  ...
  switch (sensorType) {
    case SENSOR_ACCEL: {
      ...
      if (file.readBytes((char*)&data, sizeof(data)) == sizeof(data)) {
        Serial.print(data.x, 3); Serial.print(",");
        Serial.print(data.y, 3); Serial.print(",");
        Serial.print(data.z, 3); Serial.print(",");
    ...
    case SENSOR_BARO: {
      ...
      if (file.readBytes((char*)&data, sizeof(data)) == sizeof(data)) {
        Serial.print(data.temperature, 3); Serial.print(",");
        Serial.print(data.pressure, 3); Serial.print(",");
        Serial.print(data.altitudeAboveLaunchPad, 3); Serial.print(",");
    ...
    case SENSOR_GPS: {
      ...
      if (file.readBytes((char*)&data, sizeof(data)) == sizeof(data)) {
        ...
        Serial.print(data.hdop, 2); Serial.print(",");
        Serial.print(data.satellites); Serial.print(",");
        Serial.print(data.speed, 2); Serial.print(",");
    ...
    case SENSOR_ATTITUDE: {
      ...
      AttitudeData data;
      if (file.readBytes((char*)&data, sizeof(data)) == sizeof(data)) {
        Serial.print(data.roll, 3); Serial.print(",");
        Serial.print(data.pitch, 3); Serial.print(",");
        Serial.print(data.yaw, 3); Serial.print(",");
...
bool processDataChunk(File& file, uint16_t bufferSize, uint32_t& totalBytesRead, uint16_t chunkNumber) {
  Serial.print("Chunk #"); Serial.print(chunkNumber);
  Serial.print(": "); Serial.print(bufferSize); Serial.print(" bytes");
  Serial.print(" ("); Serial.print(totalBytesRead); Serial.print("/"); Serial.print(file.size()); Serial.println(")");
  
  if (bufferSize > UNIFIED_BUFFER_SIZE || bufferSize == 0) {
    Serial.print("Error: Invalid buffer size: "); Serial.println(bufferSize);
    return false;
  }
  ...
  while (chunkBytesRead < bufferSize && file.available()) {
    ...
    chunkBytesRead += 4; totalBytesRead += 4;
    
    Serial.print(timestamp); Serial.print(",");
    
    if (!readSensorData(file, sensorType, chunkBytesRead, totalBytesRead)) return false;
    Serial.println();
    
    if (totalBytesRead >= file.size()) break;
  }
  return true;
}

void readBufferFromSD(uint8_t chipSelect) {
  if (!initSDCard(chipSelect)) return;
  
  File file = SD.open(SD_FILENAME, FILE_READ);
  if (!file) {
    Serial.print("Error: Cannot open "); Serial.print(SD_FILENAME);
    Serial.print(" for reading on SD Card "); Serial.println(getCardNumber(chipSelect));
    return;
  }
  
  uint32_t totalFileSize = file.size();
  Serial.print("=== Flight Data Dump from SD Card ("); Serial.print(totalFileSize); Serial.println(" bytes) ===");
  ...
  while (file.available() && totalBytesRead < totalFileSize) {
    ...
    if (!processDataChunk(file, bufferSize, totalBytesRead, chunkNumber++)) break;
    
    if (totalFileSize > 10000) {
      Serial.print("Progress: "); Serial.print((totalBytesRead * 100) / totalFileSize); Serial.println("%");
    }
  }
  
  file.close();
  Serial.print("=== End of Flight Data ("); Serial.print(totalBytesRead); Serial.println(" bytes processed) ===");
}
